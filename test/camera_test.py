...
USE_PWM_EN = False
ENABLE_SERVO = False  # Set True to enable servo scan behavior
...
def setup_hardware():
    ...
    pins = [Motor_ENA, Motor_IN1, Motor_IN2, Motor_IN3, Motor_IN4, Motor_ENB]
    if ENABLE_SERVO:
        pins.append(SERVO_PIN)
    for pin in pins:
...
def servo_scan_routine():
    # Toggle from ENABLE_SERVO at top of file.
    if not ENABLE_SERVO: return
