...
def control_loop(stop_evt: threading.Event, debug: bool, debug_errors: bool) -> None:
    ...
    try:
        ...
        while not stop_evt.is_set():
            ...
            # Simple safety hold (requested): if BOTH camera and front ultrasonic say "blocked",
            # do not allow any motor start/turn. Just stay stopped until cleared.
            front_blocked_now = front_now is not None and front_now < FRONT_STOP_CM
            if cam_blocked and front_blocked_now:
                stop_motors()
                resume_waiting = True
                deciding_since = None
                turning_until = None
            ...
            front_blocked = front_blocked_now
